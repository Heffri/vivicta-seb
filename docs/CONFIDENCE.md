...
- **US-style statement titles.** `Statements of earnings` / `of operations` / `of profit or loss` (plural) are locator keywords: Lundin Mining's statement of earnings was outranked by its statement of comprehensive income, Alvotech's by two prose notes.
- **Swedish decimals and YYMMDD fiscal years.** `1 478,6` is 1478.6 (a comma followed by one or two digits is a decimal, by three a thousands group), a leading `2,3` is notes 2 and 3 once the year columns are full, and `250501 − 260430` is the fiscal-2025 column (Clas Ohlson).
- **A field's own row is never extended by a stranger.** A quote printed with the field's exact label and figure (Sagax `Profit before tax 4,485`) may be corrected by the rows above it summing differently (Röko), but a failing identity is not repaired by gluing a neighbour onto it (`Profit before tax + Deferred tax`); the tax row is what gets derived (`Deferred tax + Current tax = -405`).
  Exception: a row printed as part of the field's own line may join it -- Essity's `Cost of goods sold −92,351` + `Items affecting comparability (IAC) – cost of goods sold −21` is the cost of sales that closes gross profit.
- **A known row with every column is a row, not a heading.** When the model quotes a sub-row under a row printed with the field's label, that row is the quote and its figure the value (SEB: `Operating profit 38,898` two rows above the quoted `NET PROFIT 31,063` is the bank's profit before tax; Handelsbanken's `Rörelseresultat` likewise). Only a label line without figures (ABB's `Basic earnings per share`) is a heading whose figure sits on the sub-row.
- **A defaulted operand answered null is still looked for.** Discontinued operations default to 0, so the identity fails rather than reports "missing" when the model skipped the row; the one row printed with the field's label between the other operands' rows, closing the identity in every column, is the answer (ABB `Income from discontinued operations, net of tax 174 226` under the continuing-operations subtotal).
- **Bracketed note references and `(diluted/basic)`.** `Income taxes [6]` is `Income taxes` (TRATON), and an EPS row that names both variants at once is the basic row.
- **A lone zero is never a thousands group, and zero revenue is a real answer.** `0.039` is thirty-nine thousandths (Fenix Outdoor's A-share EPS, under a heading that names "after dilution and before dilution" at once), and an investment company's printed `Net sales 0 0` (Bure Equity) passes the margin check instead of dividing by zero.
