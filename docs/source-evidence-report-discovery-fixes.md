...
> Historical record of completed work: the development notes for this change, written while it was
> in progress. The work has since reached `main`, including the merge its closing checklist
> prepares for. Kept as the audit trail, not as a description of current state.
...
3. Build the frontend and restart the desktop backend to load the Python encoding configuration. Image-only reports need the OCR language files, which are committed under `data/tessdata` (tessdata_fast, Apache-2.0); `python scripts/setup_ocr.py` remains as the repair fallback.
