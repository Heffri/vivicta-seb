...
The remaining failure was the existing five-upload concurrency test: it exposed that `useBatch`
was awaiting each `runItem` serially rather than using the documented three-wide queue. That hook
was outside this lane's assignment and is not changed by v188. The final `origin/acrylic` merge
includes v187's dedicated fix (`14773ba`, `frontend: run extra uploads three-wide`).

After that merge, the final frontend E2E rerun is **61 passed**. `npm run build` and `npm run lint`
also passed (the latter retains the same 13 advisory warnings), and the 15 fixture/offline backend
suites passed again. No model inference was requested.
