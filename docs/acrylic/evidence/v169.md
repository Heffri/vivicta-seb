...
- `GET /api/companies`, `/api/library`, `/api/kb`, and `/api/review-queue` accept
  `collection_name=midcap`; their filters use the same `collection.scope()` predicate.
...
- **Green after implementation:** `len(collection.members("midcap")) == 132`; Acast is in
  ...
  review-queue requests while retaining the Wallenberg assertions.
- Live fixture probe: Mid Cap Acast directory query returned one row; Mid Cap KB returned
  104 saved reports; Mid Cap review queue and library endpoints both returned successfully.
...
- `npm run e2e` passed: **39 tests**. The focused collection workflow also passed: **6 tests**.
- Fixture mode only; no model inference was requested.

### Screenshot

![Knowledge base filtered to SEB Mid Cap](v169/kb-midcap-dark.png)

`docs/acrylic/evidence/v169/kb-midcap-dark.png` shows the dark KB screen with
