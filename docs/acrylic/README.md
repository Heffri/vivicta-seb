...
- **Data**: a KB of **196 companies**, 95 with a stored `debt_maturity` extraction — the Wallenberg collection filter is the KB page's default (issue #4), and every entry stays visible to Ask, Compare and `eval/run.py --stored-kb` regardless of the filter.
