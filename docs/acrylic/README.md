...
passes are in `evidence/v001/` through `evidence/v009/`.
...
fetch order, prop signatures. Every piece of evidence above says the same thing: "logic change: none."
This was a restyle, not a rewrite. No router, no state library, no new runtime dependency.
...
| Change | Why | Evidence |
|---|---|---|
| `scripts/random_check.py --exclude-sector`; `locate.py` table-of-contents matching (`toc_targets`: front-matter pages, then widened to note-index pages document-wide; folio-offset alignment; +5 score boost for a TOC-named page) + schema `toc_keywords`. Two follow-ups tried and reverted: schema `exclude_keywords`, and a neighbour-page score bonus. | HANDOFF's Task 0 (`random_check` couldn't exclude sectors yet) and its "Next for a teammate" #1 (use the table of contents as a locator hint). | [v008](evidence/v008.md) |
...
the 101 already-stored source pages stayed in the top 2 throughout. Both follow-up locator experiments
were net negative: `exclude_keywords` flipped 21 companies' top-1 page (2 better, 16 worse, 3 neutral);
the neighbour-page bonus flipped 8 (radius 2) or 6 (radius 1), almost all regressions either way — both
reverted, the current locator is the TOC-only state. The label-normalization fixes: a full-KB
regression over 101 `income_statement` files found 735/736 verdicts identical, the one change inert
(doesn't affect scoring or any check); correctly-read bucket rows now score confidence 1.0 instead of
capping at 0.9. The parser fix: split-row rate (a label torn from its own figures) over 10 real PDFs
went from 12.14% → 0.24% on debt-relevant pages and 20.42% → 0.19% on income pages, with all 76
previously-verified `income_statement` quotes (+3/3 debt) still matching after the re-parse.
...
- **Fixture `/extract` ignores the section.** In fixture mode, `POST /extract` returns the same canned
  extraction regardless of `report_id` or the requested section — fine for UI work, but it means
  fixture mode alone can't distinguish per-section behavior. [v004](evidence/v004.md).
...
- **`_clean_label` doesn't strip a currency token glued to a footnote marker** (e.g. `"SEK1)"` has no
  word boundary after `SEK`, so it survives cleanup while a plain `", SEK"` doesn't). Seen once in the
  stored KB, changes nothing today, but it's the same normalization-asymmetry class as the
  digit-bearing-synonym fixes above. [v014](evidence/v014.md).
