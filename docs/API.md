...
Fetch failures include `code` (`report_unavailable`, `download_failed`, or `search_unavailable`), the compatible `tried` URL list, and `attempts: [{url, reason, kind}]` diagnostics. A completed search without verified accounts returns 404. Source download/access failures and unavailable search providers return 502 and are shown separately in Extract. Neither outcome asserts that no report exists.
