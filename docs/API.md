...
| `GET`  | `/api/companies?q=<text>` | – | `Company[]` — the listed-company directory (`data/companies.json`, Nasdaq Stockholm) plus the curated roster names it does not list, filtered by name/ticker substring; max 50. Empty `q` = first 50. Nothing is ever filtered out. Private holdings may name a parent report for company-map context, but `no_standalone_report: false` keeps their own accounts searchable. Parent coverage is not evidence that standalone accounts are unavailable. |
...
| `GET`  | `/api/kb` | – | `KbEntry[]` — everything in `data/kb/` (one per parsed report: pages indexed, sections extracted). No parameter narrows it: every saved report is always listed |
| `GET` | `/api/kb/export.csv` | `?section=debt_maturity&q=` | One CSV row per saved company extraction. Optional `q` matches the KB page's company/stem filter. No PDF or model call is needed. |
| `GET` | `/api/kb/export.pptx` | `?section=debt_maturity&q=` | A PPTX deck with one maturity-wall summary table, then one established PowerPoint slide per saved company. Filtering is identical to the whole-KB CSV and no PDF or model call is needed. |
| `GET` | `/api/kb/maturity-wall` | `?section=debt_maturity` | `MaturityWall` — deterministic upcoming-maturities list: total debt, amount due within 1 year and their share for every saved `debt_maturity` extraction, sorted comparable-first by share descending. Each row also names its `data/companies.json` sector and whether its buckets are complete, and the wall is aggregated per sector — counts plus median/min/max share over complete companies only. Reads the same decorated extracts as the CSV/PPTX exports (no PDF, no model call); 200 with `rows: []` when nothing is saved yet |
...
type Company = {
  ...
  // Parent-report context for curated private holdings; standalone availability is not inferred. `report_stem`
...
type Candidate = {          // one entity POST /api/reports/discover proposes; identity fields are model-reported unless saved
  ...
  url: string | null;       // report PDF or company-specific registry listing; /fetch tries it first
  reason: string;
...
### Restore the PDF for a saved source

`POST /api/kb/{stem}/pdf` restores the exact PDF associated with the saved report and returns `Report`. It uses the saved source URL and verifies its SHA-256 before publishing the PDF. A different edition is rejected with 409; an unreachable source returns 502. Existing saved text, extractions and reviews are preserved. The Ask source panel offers this action when its PDF is missing.

When `/api/reports/fetch` finds saved text, it also restores this exact source before considering discovery. It never substitutes another edition under existing citations. If restoration is unavailable, Extract can continue using saved text. PDFs whose hash disagrees with saved metadata are not exposed as available sources. Failed parsing does not register a report as successfully loaded.

## Company directory + on-demand report fetching

The curated roster merged into the directory is a company roster, not a promise of publicly downloadable reports. Private-company accounts may require a registry or a user-uploaded PDF. Discovery includes roster ownership context to distinguish similarly named entities; a parent's or subsidiary's accounts do not replace the requested issuer.

Fetch failures include `code` (`report_unavailable`, `report_listed`, `download_failed`, or `search_unavailable`), the compatible `tried` URL list, and `attempts: [{url, reason, kind}]` diagnostics. A completed search without a verified PDF or listing returns 404. Source download/access failures and unavailable search providers return 502. None of these outcomes asserts that no report exists.

`409` with `code: "report_listed"` means a fetched HTML page identifies the requested company and explicitly lists that year's annual report next to a download action, but no downloaded PDF passed validation. The response includes `listings: [{url, company, fiscal_year, title, evidence, access: "manual_download"}]`; `listings` is empty for other discovery failures. Extract shows an **Open report listing** link and manual download/upload instructions. Check `code` before treating a 409 as disabled downloads or protected reviews. A verified listing takes precedence over later search/download failures, but a validated PDF always returns the normal successful `Report` instead.

Discovery accepts company report archives and reputable registry/company-information pages for any company, not just Wallenberg. Listing claims must be checked against the fetched HTML; model statements alone, old-year accounts, and mentions in another issuer's page do not qualify. Ordinary PDF download endpoints need not contain `.pdf`. A listing is not registered as a report or sent to financial extraction. Verified listing URLs are remembered locally in `data/reports/report-listings.json`, keyed by normalized company and fiscal year. The source HTML is checked again on every reuse; cached metadata alone cannot confirm availability. A newly accessible PDF on that page is downloaded and validated normally. If the listing still has no PDF, discovery continues to other sources instead of ending at the cached listing. This cache is not versioned.

On desktop, **Download and continue** invokes `window.arp.downloadReport(listing)`. A separate sandboxed browser with no application preload opens the source website. The user operates the normal download button and any site verification. Completed PDF downloads are sent to `/api/reports/import-download` on the local backend. Successful validation resumes the original batch item and selected extraction section; cancellation retains the listing, and rejected PDFs display the validation error without running extraction. The IPC accepts only the application main frame, uses a separate temporary browser session, limits downloads to 50 MB, and removes its temporary file after import. CAPTCHA, authentication, and payment requirements are not bypassed. Web-only clients keep the ordinary listing link and upload flow.

Browser imports parse/OCR the downloaded PDF locally before validating issuer, fiscal year, and annual-accounts content. OCR text is reused during registration; original PDF bytes and page numbering are unchanged. Missing OCR language files return 422 with setup guidance.

Source links from the curated report index are attempted before fresh model discovery, even when the file is not cached. Link discovery recognizes accented and unaccented Swedish annual-report names and combined annual/sustainability reports. Short accounts are validated by their issuer, year, report title, income statement and balance sheet instead of being rejected solely for having 40 pages or fewer.
...
legal name, not the typed text) and its `url` is tried first for a report without saved evidence. A
search runs only that query, not other queued picks.

**AI-first fetch.** For saved evidence, PDF requests restore the original source with checksum verification; text-only requests reuse saved text without downloading. For a missing report,
...
`country` and `hint` provide optional context for the model. Search errors are reported as `502 search_unavailable` unless an independently verified listing is available.
...
`EXTRACT_SECOND_PASS` is off by default. Set `EXTRACT_SECOND_PASS=1` to opt into the bounded retry for **required** fields that are still null after the ordinary extraction (and any configured merge). It first asks for one field at a time on the locator's first two or three candidate pages with their complete extracted text and that field's schema synonyms. If that stays null, a zero-model scan ranks every untried page containing a numeric row under one of the field's `synonyms` or `row_synonyms`, and the best three pages get one final fixed-page call. Thus at most three fields and two calls per field are attempted. Both calls can write a value only when the normal quote-on-page, value-in-quote, known-label and debt scope guards accept it; otherwise the null remains a null. Accepted fields add `second_pass` to their evidence, plus `fulltext_sweep` when the scan supplied the page. Set `EXTRACT_SECOND_PASS=0` (or leave it unset) to make no follow-up calls. Extraction `timings` reports `second_pass_calls` and `second_pass` seconds alongside the aggregate `model`, `validate` and `attempts` totals.
...
### Scope, discovery, and always-on PDFs
Nothing narrows what a surface sees. GET `/api/companies`, `/api/library`, `/api/kb` and `/api/review-queue` take no scope parameter and always return everything saved, so one installation can never show fewer reports than another holding the same `data/`. The roster in `pipeline/collection.py`, sourced from Investor and FAM, is still used for two things: its names are **merged into** the company directory (private holdings such as Mölnlycke have no catalogue entry and would otherwise be unfindable), and it supplies ownership context to report discovery. It is a curated holdings list, not an exhaustive ownership graph, and it filters nothing.
...
### Historical fiscal years and alternative filings

Company discovery/fetch accepts fiscal years 1900–2100; the UI offers 1900 through the current calendar year and defaults to the preceding year. The connected search model is instructed to search historical archives and subsequent-year filings, retry alternate sources, and distinguish publication year from the requested fiscal period. Explicit annual-report titles and fiscal-year-ended declarations take priority over incidental date mentions.

When a standalone annual report is unavailable, an issuer's registration statement/prospectus may provide audited historical accounts. Acceptance requires a matching issuer in the auditor's opinion and annual balance-sheet/income-statement pages naming the requested year. These sources return optional `document_type: "registration_statement"`, `statement_pages: number[]` (original 1-based PDF pages), and `source_notice: string` on `Report`, persisted in library/KB metadata. Extraction and Ask indexing use only the verified audited section. The complete original PDF and page text remain available for source inspection. The notice appears in batch progress and extraction/Ask warnings, and is retained when reopening a saved report.

A search/access failure still does not establish non-publication. No registry, model or download failure is reported as proof that no annual report exists.
