...
for (const tone of ['light', 'dark']) test(`review guidance, one Open, statement switch and graph interactions [${tone}]`, async ({ page }) => {
  ...
  await expect(page.getByLabel('Statement', { exact: true })).toHaveValue('income_statement')
  const revenue = page.getByRole('row').filter({ has: page.getByRole('cell', { name: 'Revenue', exact: true }) })
  await expect(revenue).toContainText('Needs review')
  await expect(revenue).toContainText('Confirm the figure is in the 2025 column')
  await expect(revenue).toContainText('Confirm the table uses MSEK')
  await page.getByLabel('Statement', { exact: true }).selectOption('debt_maturity')
  await expect(page.getByRole('cell', { name: 'Total debt', exact: true })).toBeVisible()
  ...
  await page.mouse.move(canvasBox.x + 20, canvasBox.y + canvasBox.height - 30)
  await page.mouse.down()
  await page.mouse.move(canvasBox.x + 60, canvasBox.y + canvasBox.height - 60, { steps: 4 })
  await page.mouse.up()
