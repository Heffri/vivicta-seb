...
for (const tone of ['dark','light']) {
  test(`global Ask, sector map and stored sources [${tone}]`, async ({page}) => {
    ...
    await expect(page.getByRole('heading',{name:'Company map',exact:true})).toBeVisible()
    ...
    await page.getByRole('row').filter({hasText:'Atlas Copco'}).filter({hasText:'2025'}).getByRole('button',{name:'Open',exact:true}).click()
    await expect(page.getByText('Revenue 100. Saved report page.',{exact:true})).toBeVisible()
