...
test('human confirmation, correction, reopening and failed save', async ({ page }) => {
  ...
  await page.goto('/')
  const open = async () => { await page.getByRole('tab', { name: 'Knowledge base', exact: true }).click(); await page.getByRole('button', { name: 'Open', exact: true }).click() }
  await open()
  ...
  await form.getByRole('button', { name: 'Save review' }).click()
  await expect(page.getByText('Human corrected', { exact: true })).toBeVisible()
