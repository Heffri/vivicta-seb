...
import type { Extraction, Field, HumanReview } from '@/types'

export function HumanReviewForm({ extraction, field, onSaved }: { extraction: Extraction; field: Field; onSaved: (result: Extraction) => void }) {
  ...
  return <form aria-label={`Review ${field.label}`} className="space-y-3 rounded-xl border bg-card p-4" onSubmit={async e => {
    ...
    try {
      ...
      onSaved(await reviewField(extraction.report_id, { section: extraction.section, key: field.key, expected: field, decision, reviewer, note, ...(decision === 'corrected' ? { value: corrected, unit: unit.trim() || null, period: period.trim() || null } : {}) }))
      setNote('')
    ...
    {field.human_review && <p className="text-sm">Last review: {field.human_review.reviewer} · {new Date(field.human_review.at).toLocaleString()} · {field.human_review.note || field.human_review.decision}</p>}
    <fieldset disabled={busy} className="space-y-3">
      ...
      {decision === 'corrected' && <div className="grid gap-2 sm:grid-cols-3"><label className="text-sm">Value<input className={inputClass} type={typeof field.value === 'string' ? 'text' : 'number'} step="any" value={value} onChange={e => setValue(e.target.value)} placeholder="Blank = not found" /></label><label className="text-sm">Unit<input className={inputClass} maxLength={80} value={unit} onChange={e => setUnit(e.target.value)} /></label><label className="text-sm">Period<input className={inputClass} maxLength={80} value={period} onChange={e => setPeriod(e.target.value)} /></label><p className="text-xs text-muted-foreground sm:col-span-3">Use a decimal point and no thousands separators. A correction makes previous calculation checks outdated.</p></div>}
      ...
      <label className="block text-sm">Review note<textarea required={decision !== 'confirmed'} maxLength={2000} className={inputClass} value={note} onChange={e => setNote(e.target.value)} placeholder="What did you check or correct? Include the source page." /></label>
      <Button type="submit">{busy ? 'Saving…' : 'Save review'}</Button>
    </fieldset>
    {error && <p role="alert" className="text-sm text-danger">{error}</p>}
    {!!field.review_history?.length && <details><summary className="cursor-pointer text-sm">Review history ({field.review_history.length})</summary><ol className="mt-2 space-y-2 text-xs">{field.review_history.map((r, i) => <li key={i}>{r.reviewer} · {new Date(r.at).toLocaleString()} · {r.decision}<br />Previous: {String(r.previous.value ?? 'Not found')} {r.previous.unit} ({r.previous.period})<br />{r.note}</li>)}</ol></details>}
  </form>
}
