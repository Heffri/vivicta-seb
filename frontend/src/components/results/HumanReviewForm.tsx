...
export function HumanReviewForm({ extraction, field, citation, onCitationChange, onSaved, candidate, candidateWarnings = [], onDiscardCandidate, reviewerName, onReviewerNameChange }: { extraction: Extraction; field: Field; citation: { page: string; quote: string }; onCitationChange: (citation: { page: string; quote: string }) => void; onSaved: (result: Extraction) => void; candidate?: Field | null; candidateWarnings?: string[]; onDiscardCandidate?: () => void; reviewerName?: string; onReviewerNameChange?: (reviewer: string) => void }) {
  const [decision, setDecision] = useState<HumanReview['decision']>('confirmed')
  const [reviewer, setReviewer] = useState(reviewerName || field.human_review?.reviewer || '')
  ...
  return <form aria-label={`Review ${field.label}`} className="space-y-3 rounded-xl border bg-card p-4" onSubmit={async e => {
    ...
    <fieldset disabled={busy} className="space-y-3">
      ...
      <label className="block text-sm">Your name<input required maxLength={120} className={inputClass} value={reviewer} onChange={e => { setReviewer(e.target.value); onReviewerNameChange?.(e.target.value) }} /></label>
