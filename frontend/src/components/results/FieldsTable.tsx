              ...
              <Fragment key={f.key}>
                ...
                >
                  <TableCell className="whitespace-normal font-medium">
                    ...
                    {compact && !!f.components?.length && <span className="mt-1 flex flex-wrap gap-x-2 gap-y-1 text-[11px] font-normal text-muted-foreground">{f.components.map((component, index) => <button key={index} type="button" className="text-primary underline-offset-2 hover:underline" title={component.quote} onClick={event => { event.stopPropagation(); openPage(f.key, component.page) }}>{component.label || `Component ${index + 1}`} · p.{component.page}</button>)}</span>}
                  </TableCell>
