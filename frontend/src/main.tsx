...
// `settings` is SettingsView.tsx's only way to read/write the desktop config and restart the
...
declare global {
  interface Window {
    arp?: {
      ...
      // Settings' Theme select — persists config.json and switches the live window material
...
export type DesktopConfig = {
  ...
  codexModel: string
  claudeModel: string
  // EXTRACT_TWO_PASS passthrough -- ignored by desktop/settings.js's envForConfig() for
  // 'ollama' (always forced off, see its own comment) and 'fixture' (no model call either way).
  extractTwoPass: boolean
  // DEBT_BASIS passthrough -- unlike two-pass this is model-independent, so the control is shown
  // for every provider and every non-fixture provider passes it through.
  maturityBasis: 'carrying' | 'undiscounted'
  // EXTRACT_MERGE_RUNS passthrough -- the second-run merge, model-independent like the
  ...
  // EXTRACT_SECOND_PASS passthrough -- the bounded second pass + full-text sweep
  ...
  secondPass: boolean
  // Visual theme -- Solid (default, opaque surfaces) or Acrylic (glass + the desktop shell's real
  // Windows material). Persisted in config.json like maturityBasis but never an env var:
  // desktop/main.js reads it to build the window with the right background material, the renderer
  // keeps the live choice in localStorage `arp-theme`.
  theme: 'solid' | 'acrylic'
}

export type SetSettingsResult =
  // `config` is api.ts's Config — desktop main.js resolves this field from /api/config's own JSON.
  // No import cycle: api.ts imports ./types only.
...
// The theme choice from Settings' Theme select (Solid default / Acrylic glass), restored
