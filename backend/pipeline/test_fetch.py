...
def demo_private_holding_guard():
    ...
    The real Investor annual report names Sarnova in its early portfolio pages, so this guard belongs
    before both a confirmed URL and the generic source/search pipeline, not only in the UI.
    """
    ...
    # The generic issuer check is deliberately evidence-based, not an ownership graph: a parent
    # report that names its subsidiary repeatedly in the first 20 pages can satisfy it.  This small
    # Investor-shaped PDF reproduces that precondition without a model or network, proving why the
    # no-standalone guard must run *before* URL validation.
