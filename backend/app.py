...
def locate_report_pages(report_id: str, report: dict, schema: dict) -> tuple[list[str], list[int]]:
    ...
    # A verified historical filing keeps its original page numbers but only its audited annual
    # accounts are evidence.  Mask its unrelated prospectus pages before locating, then preserve
    # that mask after an on-demand OCR update (which returns the raw page-text cache).
    ...
    if wanted and pdf_path(report_id).is_file():
        texts = report_period.extraction_texts(fill_pending_ocr(report_id, report["stem"], sorted(wanted)), report)
        pages = locate.candidate_pages(texts, schema, fiscal_year=report.get("fiscal_year"))
    return texts, pages
